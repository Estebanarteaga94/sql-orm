...
The project follows an incremental release strategy. This changelog describes
the API surface available in each workspace release candidate and stable
release.

## [0.2.0-rc.1] - 2026-05-17

Pre-release candidate for the `0.2.0` line, focused on navigation properties,
stable explicit tracking, pooled transactions, migration downgrade and typed
aggregations. This candidate has passed full workspace validation, including
optional SQL Server tests in the recorded local environment.

### Etapa 20: Navigation Properties

- Navigation properties are part of the `0.2.0-rc.1` pre-release surface.
...
- Typed aggregation APIs are implemented on `DbSetQuery`: `count()`,
  `exists()`, `any()`, `sum::<T>(...)`, `avg::<T>(...)`, `min::<T>(...)`, and
  `max::<T>(...)`.
- `count()` now uses the aggregate query path and preserves explicit joins and
  root `tenant` / `soft_delete` filters. Scalar aggregates ignore ordering and
  pagination because they operate on the matching query shape before page
  slicing.
- `sum`, `avg`, `min`, and `max` return `Option<T>` because SQL Server can
  return `NULL` for empty sets or nullable expressions. The scalar materializer
  uses strict `SqlValue` to `SqlTypeMapping` matching for non-null values.
- Grouped aggregate queries are implemented with
  `group_by(...).select_aggregate(...)`, explicit aliases, DTO
  materialization through `FromRow`, SQL Server `GROUP BY`, basic `HAVING`,
  aggregate ordering and pagination.
- `try_select_aggregate(...)` rejects empty projection lists, empty aliases and
  duplicate aliases before execution. The SQL Server compiler also validates
  aggregate aliases and non-grouped expressions before SQL rendering.
- Explicit joins configured before scalar aggregates or `group_by(...)` are
  preserved, including navigation joins with explicit aliases. Aggregations do
  not infer hidden navigation joins, and manually joined entities do not receive
  automatic policy filters in this cut.
- The implementation preserves crate boundaries: aggregation structure belongs
  in `sql-orm-query`, SQL generation belongs in `sql-orm-sqlserver`, and
  execution remains in `sql-orm-tiberius`.
- Coverage includes SQL compiler snapshots, public valid and invalid
  `trybuild` fixtures, unit tests, and an optional real SQL Server test for
  scalar and grouped aggregates with DTO materialization.
...
- Navigation properties are not part of the `0.1.0` release surface. The
  implementation is released in the repository as `0.2.0-rc.1`.
- The published `0.1.0` release predates the Etapa 24 aggregation APIs; the
  repository implementation is released in the repository as `0.2.0-rc.1`.
- In `0.1.0`, `count()` used the older count path and lost explicit joins.
...
- Typed aggregations: [docs/aggregations.md](docs/aggregations.md)
