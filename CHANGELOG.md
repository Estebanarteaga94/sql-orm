...
- Experimental change tracking with `Tracked<T>`, `EntityState`, `find_tracked`, `add_tracked`, `remove_tracked`, and `save_changes`. The current implementation rejects duplicate live tracked identities for loaded rows, can reattach a detached loaded identity from registry-owned snapshots, reuses already tracked related snapshots during `load_collection_tracked(...)`, `load_collection(...)`, `include(...)` and `include_many(...)`, supports explicit detach/clear, skips no-op `Modified` updates based on persisted update payloads, orders `save_changes()` phases deterministically for simple foreign keys, validates simple-PK limits before SQL for pending work, and keeps graph relationship persistence deferred.
